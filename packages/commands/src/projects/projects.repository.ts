...
export class ProjectsRepository extends Repository {
  protected static readonly PROJECTS_FILE_PATH = path.join(
    ProjectsRepository.STORAGE_PATH,
    'projects.json',
  );

  public async findAll(): Promise<ProjectEntity[]> {
    try {
      const raw = await readFile(ProjectsRepository.PROJECTS_FILE_PATH, 'utf8');
