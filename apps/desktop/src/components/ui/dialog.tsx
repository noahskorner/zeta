...
function DialogPortal({ ...props }: React.ComponentProps<typeof DialogPrimitive.Portal>) {
  ...
  // Render dialogs inside the app body so global window chrome stays visible.
  React.useEffect(() => {
    setContainer(document.getElementById('window-body-portal'));
  }, []);
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          'bg-background data-[state=open]:animate-in data-[state=closed]:animate-out absolute top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border p-6 shadow-lg duration-200 data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95',
          className,
        )}
        {...props}
      >
