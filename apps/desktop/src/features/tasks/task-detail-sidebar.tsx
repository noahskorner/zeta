...
export function TaskDetailSidebar(props: TaskDetailSidebarProps) {
  ...
  const hasTaskContext = Boolean(props.taskId || props.taskName || props.friendlyName || props.description);
  ...
  async function handleExecuteTool(): Promise<void> {
    ...
    try {
      ...
      toast.success('Tool started.', {
        description:
          typeof receipt.pid === 'number' ? `PID ${receipt.pid}` : `Started at ${receipt.startedAt}`,
      });
    } catch (error) {
    ...
    <Sidebar side="right" collapsible="offcanvas" className="h-full">
      <SidebarHeader className="border-b">
        <div className="space-y-1">
          <div className="text-sm font-medium">Run Tool</div>
          <div className="text-xs text-muted-foreground">Execute a saved tool for this task.</div>
        </div>
      </SidebarHeader>
                ...
                <div key={`${slot}-${index}`} className="space-y-1">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      updateSlotMapping(
                        index,
                        value === 'none' ? null : (value as TaskFieldKey),
                      )
                    }
                  >
                    <SelectTrigger className="w-full">
...
function getTaskFieldValue(props: TaskDetailSidebarProps, fieldName: TaskFieldKey): string | undefined {
